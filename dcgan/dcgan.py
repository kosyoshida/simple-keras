...
class GAN():
    ...
    def conv_down(self, x, n_filters):
    ...
    def generator_model(self):
        ...
        x = Dense(7*7*128, activation = 'relu')(input_x)
        x = Reshape((7,7,128))(x)

        x = self.conv_up(x, 128)
        x = self.conv_up(x, 64)
    ...
    def discriminator_model(self):
        ...
        x = self.conv_down(input_x, 32)
        x = self.conv_down(x, 64)
        x = self.conv_down(x, 128)
        x = self.conv_down(x, 128)
...
if __name__ == '__main__':
    ...
    # model
    gan = GAN()
    gan.train(X, epochs=4000, batch_size=32)
